        ...
        activeTab === 'bookings' ? (
            ...
            <div className="space-y-8">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
                    {(() => {
                        const slots = [];
                        let current = new Date(`${format(selectedDate, 'yyyy-MM-dd')}T${workHours.start}:00`);
                        const end = new Date(`${format(selectedDate, 'yyyy-MM-dd')}T${workHours.end}:00`);
                        const duration = 60; // Assuming 60 min slots for simplicity or fetch via settings
